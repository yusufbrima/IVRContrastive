...
def main(modelstr, experiment, target_class, model_type, contrastive_method):
    ...
    classifier = FinetuningClassifier(model, num_classes=num_classes).to(device)
    ...
    torch.save(model.state_dict(), f'{MODELS_PATH}/custom_ft_{modelstr}_{target_class}_{contrastive_method}_experiment_{experiment}.pth')
    ...
    test_results_df.to_csv(f'{RESULTS_PATH}/{modelstr}_ft_test_scores_{target_class}_{contrastive_method}_experiment_{experiment}.csv', index=False)

    # Save the training history
    history_df = pd.DataFrame(history)
    history_df.to_csv(f'{RESULTS_PATH}/{modelstr}_ft_history_{target_class}_{contrastive_method}_experiment_{experiment}.csv', index=False)

    # Save the test results
    test_results = {'test_loss': test_loss, 'test_acc': test_acc}
    test_results_df = pd.DataFrame(test_results, index=[0])
    test_results_df.to_csv(f'{RESULTS_PATH}/{modelstr}_ft_test_results_{target_class}_{contrastive_method}_experiment_{experiment}.csv', index=False)
